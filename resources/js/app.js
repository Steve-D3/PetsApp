    ...
    const calendar = new Calendar(calendarEl, {
        ...
        events: async function(fetchInfo, successCallback, failureCallback) {
            try {
                ...
                const normalizedEvents = Array.isArray(events) ? events : (events.data || []);
                
                // Transform events to FullCalendar's expected format if needed
                const formattedEvents = normalizedEvents.map(event => ({
                    id: event.id,
                    title: event.title || `${event.pet?.name || 'Pet'} - ${event.type || 'Appointment'}`,
                ...
                if (errorEl && messageEl) {
                    messageEl.textContent = error.message || 'Failed to load calendar events. Please try again later.';
        ...
        eventContent: function(arg) {
            ...
            eventEl.className = `fc-event-inner fc-event-${status} ${isPast ? 'opacity-70' : ''}`;
            ...
            if (event.allDay) {
                ...
                timeEl.textContent = `${startTime}${endTime ? ` - ${endTime}` : ''}`;
            }
        ...
        dateClick: function(info) {
            ...
            const createUrl = document.getElementById('calendar')?.dataset.createUrl;
            if (createUrl) {
