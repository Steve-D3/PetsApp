<div>
                        ...
                        <div>
                            ...
                            <p class="text-lg font-semibold text-gray-900 dark:text-white">
                                @if(isset($clinicStats))
                                    {{ $clinicStats['total_appointments'] }}
                                @else
                                    0
                                @endif
                            </p>
    ...
    <div class="mt-6">
            ...
            <div
                ...
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-semibold text-gray-900 dark:text-white">
                            {{ is_array($pendingTasks) ? count($pendingTasks) : ($pendingTasks ?? 0) }}
                        </p>
                    </div>
                </div>
                <div class="mt-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <div
                ...
                <div class="flex items-center justify-between mb-6">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <span
                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ count($pendingTasks) > 0 ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' : 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' }}">
                            {{ count($pendingTasks) }} {{ Str::plural('Task', count($pendingTasks)) }}
                        </span>
                    </div>
                </div>
                    ...
                    @forelse($pendingTasks as $task)
                        <div class="group flex items-start p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                            <button type="button"
                                ...
                                class="flex-shrink-0 mt-0.5 flex items-center justify-center h-5 w-5 rounded-full border-2 {{ $task->completed ? 'bg-green-100 border-green-500 text-green-600 dark:bg-green-900/50 dark:border-green-500 dark:text-green-400' : 'border-gray-300 dark:border-gray-500' }}"
                            ...
                            <div class="ml-3 flex-1 min-w-0">
                                <div class="flex justify-between items-start">
                                    <p class="text-sm font-medium {{ $task->completed ? 'text-gray-500 dark:text-gray-400 line-through' : 'text-gray-900 dark:text-white' }}">
                                        {{ $task->title }}
                                    </p>
                                    <div class="flex items-center space-x-2">
                                ...
                                <div class="mt-1.5 flex items-center text-xs text-gray-500 dark:text-gray-400">
                                    ...
                                    <span class="{{ $task->due->isToday() ? 'font-medium text-amber-600 dark:text-amber-400' : '' }}">
                                        {{ $task->due->isToday() ? 'Today' : $task->due->format('M j, Y') }} • {{ $task->due->format('g:i A') }}
                                    </span>
                                    @if(!$task->due->isFuture())
                                        <span class="ml-2 px-1.5 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300">
                                            {{ $task->due->isToday() ? 'Due today' : ($task->due->isPast() ? 'Overdue' : '') }}
                                        </span>
                                    @endif
                                </div>
                            </div>
                ...
                @if(count($pendingTasks) > 0)
                    <div class="mt-4 flex justify-between items-center border-t border-gray-200 dark:border-gray-700 pt-4">
                        ...
                        <span class="text-xs text-gray-500 dark:text-gray-400">
                            {{ count(array_filter($pendingTasks, fn($t) => $t->completed)) }} of {{ count($pendingTasks) }} completed
                        </span>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl shadow-sm border border-gray-100/50 dark:border-gray-700/50 p-6 hover:shadow-md transition-shadow duration-300 h-full flex flex-col">
                <div class="flex items-center justify-between mb-4">
                    ...
                    @if($vaccineStats['due_soon'] > 0)
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-200">
                            {{ $vaccineStats['due_soon'] }} {{ Str::plural('Due', $vaccineStats['due_soon']) }}
                        </span>
                    @endif
                </div>
                <div class="flex-1">
                        ...
                        <div class="space-y-4">
                                ...
                                <div class="flex">
                                    ...
                                    <div class="ml-3">
                                        <h3 class="text-sm font-medium text-yellow-800 dark:text-yellow-200">
                                            {{ $vaccineStats['due_soon'] }} {{ Str::plural('Vaccination', $vaccineStats['due_soon']) }} Due
                                        </h3>
                                        <p class="mt-1 text-xs text-yellow-700/90 dark:text-yellow-300/90">
                                            Due or overdue for {{ count($vaccineStats['due_vaccinations']) }} {{ Str::plural('pet', count($vaccineStats['due_vaccinations'])) }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flow-root max-h-64 overflow-y-auto pr-2 -mr-2">
                                <ul role="list" class="divide-y divide-gray-200/50 dark:divide-gray-700/50">
                                    @foreach($vaccineStats['due_vaccinations'] as $petVaccinations)
                                                ...
                                                <div class="flex-shrink-0 mt-1">
                                                    <span class="inline-flex items-center justify-center h-9 w-9 rounded-full bg-indigo-100/70 dark:bg-indigo-900/30">
                                                        <span class="text-sm font-medium text-indigo-600 dark:text-indigo-300">
                                                            {{ substr($petVaccinations['pet_name'], 0, 1) }}
                                                        </span>
                                                    </span>
                                                </div>
                                                <div class="ml-3 flex-1 min-w-0">
                                                    <div class="flex justify-between items-start">
                                                        <p class="text-sm font-medium text-gray-900 dark:text-white truncate">
                                                            {{ $petVaccinations['pet_name'] }}
                                                        </p>
                                                        <a href="{{ route('admin.pets.show', $petVaccinations['pet_id']) }}"
                                                    ...
                                                    <div class="mt-1 space-y-1.5">
                                                        @foreach($petVaccinations['vaccinations'] as $vax)
                                                            <div class="flex flex-wrap items-center gap-1.5">
                                                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap {{ $vax['is_overdue'] ? 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-200' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-200' }}">
                                                                    {{ $vax['name'] }}
                                                                </span>
                                                                <span class="text-xs {{ $vax['is_overdue'] ? 'text-red-600 dark:text-red-400' : 'text-yellow-600 dark:text-yellow-400' }} whitespace-nowrap">
                                                                    {{ $vax['is_overdue'] ? 'Overdue' : 'Due' }} {{ $vax['due_date'] }}
                                                                    @if(!$vax['is_overdue'])
                                                                        <span class="text-gray-500 dark:text-gray-400">(in {{ $vax['days_until_due'] }}d)</span>
                                                                    @endif
                                                                </span>
                                                            </div>
                                                        @endforeach
                ...
                <div class="mt-4 pt-4 border-t border-gray-200/50 dark:border-gray-700/50">
                    <a href="{{ route('vet.patients.index') }}"
                                        ...
                                        <div class="flex items-center space-x-4 min-w-0">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <div class="flex items-center mt-1 text-sm text-gray-500 dark:text-gray-400">
                                                    <span class="truncate">{{ $record->pet->name }}</span>
                                                    <span class="mx-1">•</span>
                                                    <span class="capitalize">{{ $record->pet->species }}</span>
                                                </div>
                                            </div>
                                        </div>
        ...
        <!-- All Tasks Modal -->
        <x-dialog-modal wire:model.live="showAllTasksModal">
            <x-slot name="title">
                <div class="flex items-center">
                    ...
                    @forelse($pendingTasks as $task)
                        <div class="flex items-start p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            <div class="flex-shrink-0 pt-0.5">
                                <div class="h-5 w-5 rounded-full flex items-center justify-center {{ $task->priority === 'high' ? 'bg-red-100 text-red-600 dark:bg-red-900/50 dark:text-red-400' : 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/50 dark:text-yellow-400' }}">
                                    <svg class="h-3 w-3" fill="currentColor" viewBox="0 0 20 20">
