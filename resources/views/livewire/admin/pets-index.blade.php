                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-600 bg-white dark:bg-gray-800">
                                    @foreach ($pets as $pet)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                                ...
                                                <div class="flex items-center">
                                                    <div class="h-10 w-10 flex-shrink-0">
                                                        <div class="h-10 w-10 overflow-hidden rounded-full">
                                                            <img src="{{ $pet->photo ? $pet->photo : 'https:/picsum.photos/id/237/200/200' }}" alt="Pet Photo" class="h-10 w-10 object-cover">
                                                        </div>
                                                    </div>
                                                    <div class="ml-4">
                                                        <div class="font-medium text-gray-900 dark:text-white">{{ $pet->name }}</div>
                                                        <div class="text-gray-500 dark:text-gray-400">{{ $pet->weight ? $pet->weight . ' kg' : 'N/A' }}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-300">
                                                <div class="text-gray-900 dark:text-white">{{ $pet->owner?->name ?? 'No owner' }}</div>
                                            ...
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-300">
                                                {{ $pet->birth_date ? $this->calculateAge($pet->birth_date) . ' years' : 'N/A' }}
                                            </td>
                                            ...
                                            <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                                <div class="flex items-center space-x-2">
                                                    <a href="{{ route('admin.pets.show', $pet->id) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300">
                                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                        </svg>
                                                    </a>
                                                    <a href="{{ route('admin.pets.edit', $pet->id) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">
                                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                        </svg>
                                                    </a>
                                                    <button wire:click="delete({{ $pet->id }})" wire:confirm="Are you sure you want to delete this pet?" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300">
                                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                <div>
                                    <p class="text-sm text-gray-700 dark:text-gray-300">
                                        Showing
                                        <span class="font-medium">{{ $pets->firstItem() }}</span>
                                        to
                                        <span class="font-medium">{{ $pets->lastItem() }}</span>
                                        of
                                        <span class="font-medium">{{ $pets->total() }}</span>
                                        results
                                    </p>
                                </div>
                                <div>
                                    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                        ...
                                        @foreach ($pets->links()->elements[0] as $page => $url)
                                            ...
                                            @else
                                                <button wire:click="gotoPage({{ $page }})" class="bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 relative inline-flex items-center px-4 py-2 border text-sm font-medium">
                                                    {{ $page }}
                                                </button>
                                            @endif
                                        @endforeach
