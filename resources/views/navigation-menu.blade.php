                    ...
                    <div class="flex items-center">
                        ...
                        <div class="shrink-0 flex items-center">
                            <a href="{{ route('home') }}" class="flex items-center space-x-2 group">
                                <x-application-mark
                                    class="block h-8 w-auto text-indigo-600 dark:text-indigo-400 transition-transform duration-200 group-hover:scale-105" />
                                <span
                        ...
                        <div class="hidden sm:ml-10 sm:flex sm:space-x-1">
                            @auth
                                @if (auth()->user()->role === 'admin')
                                    <x-nav-link href="{{ route('admin.dashboard') }}"
                                        ...
                                        class="group relative px-3 py-2 rounded-lg transition-all duration-200 hover:bg-gray-100 dark:hover:bg-gray-800">
                                ...
                                @elseif (auth()->user()->role === 'vet')
                                    ...
                                    <x-nav-link href="{{ route('vet.appointments.index') }}"
                                        :active="request()->routeIs('vet.appointments.*')">
                                        {{ __('Appointments') }}
                                    </x-nav-link>
                                    <x-nav-link href="{{ route('vet.patients.index') }}"
                                        :active="request()->routeIs('vet.patients.*')">
                                        {{ __('My Patients') }}
                                    </x-nav-link>
                                @endif
                            @endauth
                            @if (auth()->user()->role === 'admin')
                                ...
                                <x-nav-link href="{{ route('admin.clinics.index') }}"
                                    :active="request()->routeIs('admin.clinics.*')"
                                    class="group relative px-3 py-2 rounded-lg transition-all duration-200 hover:bg-gray-100 dark:hover:bg-gray-800">
                                    <span class="flex items-center space-x-2">
                                        <svg class="w-5 h-5 text-gray-500 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="pt-2 pb-3 space-y-1">
                    @auth
                        @if (auth()->user()->role === 'admin')
                            ...
                            <x-responsive-nav-link href="{{ route('admin.vets.index') }}"
                                :active="request()->routeIs('admin.vets.*')">
                                {{ __('Vets') }}
                            </x-responsive-nav-link>
                            <x-responsive-nav-link href="{{ route('admin.clinics.index') }}"
                                :active="request()->routeIs('admin.clinics.index')">
                                {{ __('Clinics') }}
                            </x-responsive-nav-link>
                        @elseif (auth()->user()->role === 'vet')
                            <x-responsive-nav-link href="{{ route('vet.dashboard') }}"
                                :active="request()->routeIs('vet.dashboard')">
                                {{ __('Dashboard') }}
                            </x-responsive-nav-link>
                            <x-responsive-nav-link href="{{ route('vet.appointments.index') }}"
                                :active="request()->routeIs('vet.appointments.*')">
                                {{ __('Appointments') }}
                            </x-responsive-nav-link>
                            <x-responsive-nav-link href="{{ route('vet.patients.index') }}"
                                :active="request()->routeIs('vet.patients.*')">
                                {{ __('My Patients') }}
                            </x-responsive-nav-link>
                            @if(auth()->user()->veterinarianProfiles->isNotEmpty())
                                <x-responsive-nav-link
                                    href="{{ route('vet.appointments.calendar', ['veterinarian_profile_id' => auth()->user()->veterinarianProfiles->first()->id]) }}"
                                    :active="request()->routeIs('vet.appointments.calendar')">
                                    {{ __('Calendar') }}
                                </x-responsive-nav-link>
                            @endif
                        @endif
